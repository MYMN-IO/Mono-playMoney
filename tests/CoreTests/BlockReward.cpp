...
bool gen_block_reward::check_block_rewards(CryptoNote::Core& /*c*/, size_t /*ev_index*/,
  ...
  for (size_t i = 0; i < blk_rewards.size(); ++i) {
    if(parameters::SECOND_BLOCK_REWARD && m_currency->moneySupply() == parameters::MONEY_SUPPLY - parameters::GENESIS_BLOCK_REWARD) {
      blk_rewards[i] = parameters::SECOND_BLOCK_REWARD;
    } else {
