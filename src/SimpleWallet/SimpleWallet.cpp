...
const command_line::arg_descriptor<uint16_t> arg_daemon_port = { "daemon-port", "Use daemon instance at port <arg> instead of 18236", 0 };
...
std::string tryToOpenWalletOrLoadKeysOrThrow(LoggerRef& logger, std::unique_ptr<IWalletLegacy>& wallet, const std::string& walletFile, const std::string& password) {
  ...
  if (walletExists) {
    ...
    if (initError) { //bad password, or legacy format
      logger(WARNING) << "Error while opening wallet: " << initError.message();
      logger(WARNING, BRIGHT_RED) << "If you are sure the wallet file is okay and the password is correct, then maybe it was created or opened by the GUI wallet - files touched by the GUI wallet are not compatible with simplewallet!";
      if (keysExists) {
        std::stringstream ss;
